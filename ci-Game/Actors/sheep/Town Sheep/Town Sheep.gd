...
var player
var slimes_collected 

@onready var tile_map = get_parent()
@onready var quest_area = $quest_area as Area2D
@onready var sheep_quests = $"Sheep Quests" 
...
func _physics_process(_delta):
	...
	if in_quest == false and chatting == false:
		sheep_movement()
	
	quest()
	
	if sheep_quests != null:
		if sheep_quests.quest1_active:
			in_quest = true
			$AnimatedSprite2D.play("idle")
	
	if player != null:
		slimes_collected = player.slimes_collected
...
func quest():
	if Input.is_action_just_pressed("quest") and in_quest_area:
		if chatting == false:
			sheep_quests.open_quest()
			chatting = true
			$AnimatedSprite2D.play("idle")
			
		else:
			sheep_quests.close_quest()
			chatting = false
...
func _on_sheep_quests_quest_menu_closed():
	chatting = false
